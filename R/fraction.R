...
#'
#' The fraction subtraction data was originally described by Tatsuoka (1990) to
#' introduce the rule space model, and later by Tatsuoka (2002) demonstrate the
#' use of cognitive modeling in educational testing.
#' The data contains responses from 536 respondents to 20 items which ask about
#' different skills related to the subtraction of fractions.
...
#' * `borrow_numerator`: Column borrow to subtract the second numerator from the
#'   first.
#' * `subtract`: Subtract numerators.
#' * `reduce`: Reduce the answer to its simplest form.
#'
#' @format `fraction_data` is a [tibble][tibble::tibble-package] containing
...
#' @references Tatsuoka, C. (2002). Data analytic methods for latent partially
#'   ordered classification models. *Journal of the Royal Statistical Society:
#'   Series C (Applied Statistics), 51*(3), 337-350.
#'   \doi{10.1111/1467-9876.00272}
#' @references Tatsuoka, K. K. (1990). Toward an integration of item-response
#'   theory and cognitive error diagnosis. In N. Frederiksen, R. Glaser, A.
#'   Lesgold, & M. G. Shafto (Eds.), *Diagnosing monitoring of skill and
#'   knowledge acquisition* (pp. 453-488). Lawrence Erlbaum Associates.
#' @rdname fraction
#' @aliases fraction
"fraction_data"

#' @format `fraction_qmatrix` is a [tibble][tibble::tibble-package] that
